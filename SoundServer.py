...
class SoundObject:
    ...
    def __init__(self, sound_=None, priority_=0,
        ...
        #    'Expecting pygame sound player, got %s ' % type(sound_)
...
class SoundControl:
    ...
    def __init__(self, gl_, channel_num_=8):
    ...
    def show_sounds_playing(self) -> None:
        """
        ...
        for object_ in self.snd_obj:
            if object_ is not None:
                print('Name :', object_.name, 'id ', object_.id, ' priority ', object_.priority,
                      ...
                      ' time left : ', round(self.snd_obj[j_].length - (time.time() - self.snd_obj[j_].time), 2))
            j_ += 1
    ...
    def stop_all_except(self, exception: int = None) -> None:
        ...
        for ch in self.all:
            lch = ch - self.start
            snd_object = self.snd_obj[lch]
            if snd_object is not None and hasattr(snd_object, 'obj_id'):
    ...
    def stop_name(self, name_: str = "") -> None:
    ...
    def stop_object(self, object_id) -> None:
    ...
    def play(self, sound_: pygame.mixer.Sound, loop_=False, priority_=0, volume_=1,
             ...
             x_=0, object_id_=None, screenrect_=pygame.Rect(0, 0, 800, 1024)):
        """
        :param screenrect_: SCREENRECT
        :type loop_: bool
        ...
        try:
            ...
            lch = self.channel - self.start
            if self.channels[lch].get_busy() == 0:
                ...
                self.channels[lch].play(sound_, loops=-1 if loop_ else 0, maxtime=0, fade_ms=fade_out_ms)

                self.channels[lch].set_volume(volume_)
                self.snd_obj[lch] = SoundObject(sound_, priority_, name_, self.channel, object_id_)

                # play a sound in stereo
                if panning_:
                    self.channels[lch].set_volume(
                        self.stereo_panning(x_, screenrect_)[0] * volume_,
                        self.stereo_panning(x_, screenrect_)[1] * volume_)
            ...
            if 'lch' in globals():
                print(self.channel, lch)

    @staticmethod
    def stereo_panning(x_, screenrect_) -> tuple:
        # assert isinstance(x_, (float, int)), 'Expecting float got %s ' % type(x_)
        if x_ < 0:
            # do not play sound for objects outisde the screen
            return 1, 0

        elif x_ > screenrect_.w:
            return 0, 1

        else:
            if screenrect_.w != 0:
                right_volume = float(x_) / screenrect_.w
            else:
                right_volume = 0.5
